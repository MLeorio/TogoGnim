...
export class AuthService {
  ...
  async signup(signupDto: Partial<RegisterDto>) {
    ...
    if (password != password2)
      throw new ConflictException('Les mots de passe ne sont pas conformes ! ');
  ...
  async resetPassword(resetPasswordDto: ResetPasswordDto) {
    ...
    return {
      message:
        "L'email de reinitialisation du mot de passe a été bien envoyé !",
    };
  }
  ...
  async resetPasswordConfirm(token: string, newPassword: string) {
    ...
    if (!user)
      throw new NotFoundException('Le lien de reinitialisation a expiré');
